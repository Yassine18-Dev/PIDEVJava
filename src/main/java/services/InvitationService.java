...
public class InvitationService {
    ...
    public void sendInvitation(Team team, Player target, int senderId, String message) throws SQLException {
        ...
        String sql = "INSERT INTO invitation(team_id, player_id, sent_by, status, message) VALUES (?,?,?,'PENDING',?)";
    ...
    public boolean acceptInvitation(int invitationId, int playerId) throws SQLException {
        ...
        try {
            ...
            try (PreparedStatement ps = cnx.prepareStatement(
                ...
                ResultSet rs = ps.executeQuery();
                if (!rs.next()) throw new IllegalStateException("Invitation introuvable.");
                inv = mapBasic(rs);
            }
            ...
            try (PreparedStatement ps = cnx.prepareStatement(
                    "SELECT current_players, max_players FROM team WHERE id=? FOR UPDATE")) {
            ...
            // ===== NOTIFICATION DISCORD APRÈS COMMIT (best-effort) =====
            try {
                Team   team   = _teamSvc.getById(inv.getTeamId());
                Player player = _playerSvc.getById(inv.getPlayerId());
    ...
    public int countPendingForPlayer(int playerId) throws SQLException {
        try (PreparedStatement ps = cnx.prepareStatement(
                "SELECT COUNT(*) FROM invitation WHERE player_id=? AND status='PENDING'")) {
            ps.setInt(1, playerId);
            ResultSet rs = ps.executeQuery();
            return rs.next() ? rs.getInt(1) : 0;
        }
    ...
    public List<Invitation> findInvitationsForPlayer(int playerId) throws SQLException {
        String sql = "SELECT i.*, t.name AS team_name, s.username AS sender_name, p.username AS player_name " +
                "FROM invitation i JOIN team t ON t.id = i.team_id " +
                "JOIN player s ON s.id = i.sent_by JOIN player p ON p.id = i.player_id " +
